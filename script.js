function aggregateData(maaltider) {
  ...
  fetchContent("Maaltider.json").then((data) =>{
    ...
    data.tallerken.forEach(maaltid => {
        if (maaltid.maaltid_id === 1 || maaltid.maaltid_id === 4 || maaltid.maaltid_id === 6) {
            maaltidVegetar.push(maaltid);
        } else if (maaltid.maaltid_id === 2 || maaltid.maaltid_id === 3 || maaltid.maaltid_id === 5) {
  ...
  createBarChart(aggregatedVegetarData, "#VegetarianBarChart", "rgb(255,201,7)", "rgb(204,131,4)");
  createBarChart(aggregatedMeatloverData, "#MeatBarChart", " rgb(0,195,255)", "rgb(0,75,99)");
  ...
  function createBarChart(aggregatedData, elementSelector, lightcolor, darkcolor) {
    const margin = {top: 20, right: 20, bottom: 50, left: 100},
        width = 600 - margin.left - margin.right,
        height = 400 - margin.top - margin.bottom;
  
    const data = Object.entries(aggregatedData).map(([category, value]) => ({ category, value }));
    ...
    svg.selectAll("text.value-label")
        ...
        .attr("x", d => x(d.value) + 3) // a little offset to the right of the bar's end
  ...
  vegetarianptag.textContent = "Resultat er baseret på en vegetarisk diæt for en hel dag. Diæten består af Morgenmad: 170g Müsli, 210g Sødmælk, 30 Blåbær, 30g Jordbær. Frokost: 200g Rudbrød, 140g Æg, 10g Pesto, 10g Soltørrede tomater. Aftensmad: 100g Ris, 120g Falafel, 80g Rucola Salat, 100g Rødvin.";
  ...
  meatptag.textContent = "Resultatet er baseret på en normal diæt for en hel dag. Diæten består af Morgenmad: 120g Grovbolle, 50g Danbo Ost, 20g Roastbeef, 250g Appelsinjuice. Frokost: 200g Rugbrød, 80g Leverpostej, 60g Rødbede, 20g Agurk. Aftensmad: 150g Hakket Grisekød, 100g Kartoffeler, 50g Broccoli, 100g Hvidvin.";
  ...
  /*
  appendImg("#sortvegetarian","menu.png");
  appendImg("#sortmeatlover","menu.png");
  
  d3.selectAll(".sort-billede")
  .on("mouseover",function(event,d){
    d3.select(this)
    .style("opacity", 0.5);
  })
  ...
  d3.selectAll(".sort-billede img")
  .on("click", showOverlayContentSort);

  d3.select('#year')
  .on('click', function(event,d){
    console.log('Year button clicked');
    yearlyEmissionVegetarian = calculateYearlyEmissions(aggregatedVegetarData);
    yearlyEmissionMeatLover = calculateYearlyEmissions(aggregatedMeatloverData);
    UpdateBarChart(yearlyEmissionVegetarian, "#VegetarianBarChart", "rgb(255,201,7)", "rgb(204,131,4)");
    UpdateBarChart(yearlyEmissionMeatLover, "#MeatBarChart", " rgb(0,195,255)", "rgb(0,75,99)");
  
  });  


  function showOverlayContentSort() {
    d3.select('#overlayContentSort').style('display', 'block');
  }
  
  function closeOverlayContentSort() {
    d3.select('#overlayContentSort').style('display', 'none');
  }   

  function UpdateBarChart(yearlyEmission, elementSelector, lightcolor, darkcolor) {
    ...
    const data = Object.entries(yearlyEmission).map(([category, value]) => ({ category, value }));
