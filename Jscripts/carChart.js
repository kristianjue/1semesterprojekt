function carChart() {
  ...
  // add click events to information icons to display car information
  d3.selectAll(".information-picture img").on(
    "click",
    showOverlayContentInfocar
  );

  // add click event to close the overlay, when the close button is clicked
  d3.select("#closeOverlaycar").on("click", closeOverlayContentInfocar);

  //add click events for displaying information
  let billede = document.getElementsByClassName("information-picture");

  billede[0].addEventListener("click", function () {
    // Handle click for electric car information
  ...
  billede[1].addEventListener("click", function () {
    // Handle click for fuel car information
  ...
  // function to display information in the overlay
  ...
  //function to show the overlay content for car information
  function showOverlayContentInfocar() {
    d3.select("#overlayContentcar").style("display", "block");
  }

  //function to close the overlay content for car information
  function closeOverlayContentInfocar() {
    d3.select("#overlayContentcar").style("display", "none");
  }

  //function to create and display bar chart for fuel car
  function barchartel(dataset) {
    ...
    //Defines x scales based on the dataset
    let x = d3
      .scaleLinear()
      .domain([
    ...
    //Defines y scales based on the dataset
    let y = d3
      .scaleBand()
      .range([0, height])
    ...
    // create and style the bars in the bar chart for electric car data:
    svg
      .selectAll("rect")
      .data(dataset)
      .enter()
      ...
      .attr("class", "eCar")
      //Defines behavior on mousemove event
      ...
      //Defines behavior on mouseout event
      .on("mouseout", function (event, d) {
    ...
    svg
      ...
      .style("fill", function (d) {
        if (d[0] > 0) {
          ...
          // Set yellow for negative values
